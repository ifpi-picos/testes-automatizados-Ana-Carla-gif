...
// UTILIZA O THIS PARA ACESSAR O QUE ESTÁ DENTRO DOS PRIVADOS
  ...
  public constructor(){ // UTILIZA-SE ISSO PARA NÃO MOSTRAR OS TRACINHOS VERMELHOS (ERRADO)
    this.numeroConta = 0
    this.agencia = 0
    this.saldo = 0
    this.extrato =[]
  } 
  public depositar (valor : number){ // publico para aparecer para o usuário
    if (valor > 0) {
      this.saldo += valor ;
  ...
  private receberTranferencia ( valor: number, conta:ContaBancaria) { // EStá privado pq o usuário ñ vai acessar diretamente.
